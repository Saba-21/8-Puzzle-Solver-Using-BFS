...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while(!e.BFSQueue.isEmpty()){                           //BFS             
            String current = e.BFSQueue.remove();         
            e.move(current);                  
        }
        
        System.out.println("Error!, Algorithm Can't Find Solution");    
    }
...
void add(String old, String next){              //add new nodes to tree
...
void check(String old, String next) {           //check completion
    
    add(old,next);  
    
    if(next.equals("123456780")) {              //solution is found
        
        String solution = next;
        
        while (solution != null) {              //get all the steps 
            steps.add(solution);
        ...
        for(int i = 0; i < steps.size(); i++){    //print steps
...
void move(String current){
    ...
    if(a!=0 && a!=3 && a!=6){                   // Move the blank space left
...
void print(String s){                           //print string as 3x3 matrix
...
String swap(int a, int b, String s){            //move blank space to next position
